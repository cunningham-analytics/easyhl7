...
1.  **HL7 grammar** - version/message configuration describes groups,
    cardinality, anchors, preambles, and children.
2.  **Feed-specific behavior** - small overrides describe deviations
    from the standard.
3.  **Parsing engine** - generic dbt macros reconstruct hierarchy and
    parse groups without knowing message-specific group names.
4.  **Consumer models** - small dbt models choose which HL7 groups
...
EasyHL7 is currently under active development.

The public API, supported HL7 versions/message types, configuration
format, and output representation may change as additional real-world
feeds and message structures are tested.
